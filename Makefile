...
build-acceptance-servers: ## Build Acceptance Servers
	...
	@docker build backend -t danalvrz/ploneconf-backend:acceptance -f backend/Dockerfile.acceptance
	@echo "Build acceptance frontend"
	@docker build frontend -t danalvrz/ploneconf-frontend:acceptance -f frontend/Dockerfile

.PHONY: start-acceptance-servers
start-acceptance-servers: build-acceptance-servers ## Start Acceptance Servers
	@echo "Start acceptance backend"
	@docker run --rm -p 55001:55001 --name ploneconf-backend-acceptance -d danalvrz/ploneconf-backend:acceptance
	@echo "Start acceptance frontend"
	@docker run --rm -p 3000:3000 --name ploneconf-frontend-acceptance --link ploneconf-backend-acceptance:backend -e RAZZLE_API_PATH=http://localhost:55001/plone -e RAZZLE_INTERNAL_API_PATH=http://backend:55001/plone -d danalvrz/ploneconf-frontend:acceptance
